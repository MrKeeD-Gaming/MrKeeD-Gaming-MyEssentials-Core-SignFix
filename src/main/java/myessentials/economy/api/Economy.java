...
public class Economy {
    ...
    public Economy(String costItemName) {
        ...
        if (costItemName.equals(CURRENCY_VAULT)) {
            ...
            if (econManagerClass == null) throw new EconomyException("Failed to initialize Vault economy!");
        } else if (costItemName.equals(CURRENCY_FORGE_ESSENTIALS)) {
            if (Loader.isModLoaded("ForgeEssentials")) econManagerClass = ForgeessentialsEconomy.class;
            if (econManagerClass == null) throw new EconomyException("Failed to initialize ForgeEssentials economy!");
    ...
    /**
     * Takes the amount of money specified. Returns false if player doesn't have the money necessary
     */
